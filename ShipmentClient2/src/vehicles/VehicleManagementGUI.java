...
public class VehicleManagementGUI extends JFrame {
    ...
    private void loadVehicles() {
        ...
        new Thread(() -> {
            try {
                ...
                ServerResponse resp = conn.sendFunction(new ClientRequest(ClientRequest.GET_VEHICLES, null));
                conn.close();

                if(resp.getData() instanceof List<?> vehicles) {
    ...
    private void addVehicle() {
        try {
            ...
            new Thread(() -> {
                try {
                    ClientConnection conn = new ClientConnection();
                    ServerResponse resp = conn.sendFunction(new ClientRequest(ClientRequest.ADD_VEHICLE, v));
                    conn.close();
    ...
    private void updateVehicle() {
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ClientConnection conn = new ClientConnection();
                    ServerResponse resp = conn.sendFunction(new ClientRequest(ClientRequest.UPDATE_VEHICLE, v));
                    conn.close();
                    SwingUtilities.invokeLater(() -> {
                        JOptionPane.showMessageDialog(this, resp.getMessage());
                        loadVehicles();
    ...
    private void deleteVehicle() {
        ...
        new Thread(() -> {
            try {
                ClientConnection conn = new ClientConnection();
                ServerResponse resp = conn.sendFunction(new ClientRequest("DELETE_VEHICLE", id));
                conn.close();
